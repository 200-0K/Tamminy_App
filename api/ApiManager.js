...
const updateAxiosToken = (axiosInstance, token, tokenChangedCallback = () => {}) => {
...
export const ApiManager = options => {
  ...
  axiosInstance.interceptors.response.use(
    response => {
      if (response.data.token) updateAxiosToken(axiosInstance, response.data.token, options.tokenChangedCallback)
